...
module.exports = class UserController {
  ...
  static async getAllPetsByUser(req, res) {
    // get user by token
    const token = getToken(req);
    const user = await getUserByToken(token);
  ...
  static async schedule(req, res) {
    ...
    // check if user has alredy scheduled a visit
    if (pet.adopter) {
    ...
    if (!pet.available) {
      return res.status(200).json({
        message: "Desculpe, o pet selecionado não está mais disponível!",
      });
    }
  ...
  static async concludeAdoption(req, res) {
    ...
    const pet = await Pet.findById(id);
    if (!pet) {
      res.status(404).json({ message: "Desculpe não encontramos esse pet!" });
    }
